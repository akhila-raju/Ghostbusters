...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        pacmanAction = None
        closestMazeDist = float('inf')
        for dist in livingGhostPositionDistributions:
            maxProbability  = dist.argMax()
            for a in legal:
                successorPosition = Actions.getSuccessor(pacmanPosition, a)
                mazeDist = self.distancer.getDistance(successorPosition, maxProbability)
                if mazeDist < closestMazeDist:
                    pacmanAction = a
                    closestMazeDist = mazeDist
            return pacmanAction
